...
from gatet0 import Tele  # <-- original gatet0.py
...
def get_proxies():
    # Not used by gatet0, but kept for compatibility
    return None
...
# ==========================================
success_keys = [
    "successful", "thank you", "donation successful", "thank", "success", "Thank you", "succeeded", "successfully"
    "appreciate", "payment success", "confirmation",
...
cvv_keys = ["security code", "incorrect cvc", "cvc", "cvv", "incorrect security code", "Your card does not support this type of purchase"]
...
otp_keys = ["Verifying strong customer authentication. Please wait...", "action_required", "verifying", "call_next_method", "requires_source_action", "requires_action", "3d_secure", "authenticate"]
...
# ---------- Send a result message for LIVE cards only ----------
def send_live_result(chat_id, cc, status_label, response, amount, elapsed):
    """Only call this for statuses: CHARGED, 3DS/OTP, LOW FUNDS."""
...
# ---------- check_card – now sends only live ----------
def check_card(cc, session):
    if not session['is_running']: return

    start_time = time.time()
    try:
        raw_response = Tele(cc)   # original gatet0 returns response.text
    ...
    # --- Parse and classify the raw response ---
    try:
        ...
        if any(k in full_text.lower() for k in ["insufficient", "not enough", "low funds"]):
            ...
            if 'errors' in j:
                ...
                if isinstance(err, dict):
                    message = " ".join([str(v) for v in err.values()])
                else:
                    message = str(err)
            elif 'message' in j:
                message = j['message']
    ...
    amt = parts[-2].strip() if len(parts) >= 2 else "0.50"
    ...
    # ---------- Determine status_label and update counters ----------
    ...
    # ========================================================
    # Anti-Spam / CVV Error Detection – SKIP FOR ADMIN
    # ========================================================
    if session['user_id'] != ADMIN_ID:                # <-- FIX: admin is exempt
...
def single_check(message):
    ...
    def do_check():
        ...
        try:
            raw = Tele(card)
            elapsed = round(time.time() - start_time, 1)
            # Classify raw response – same logic as in check_card
            try:
                j = json.loads(raw)
            ...
            # For single /chk, we show it anyway (user expects a result)
