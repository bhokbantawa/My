...
def check_card(cc, session):
    ...
    try:
        ...
        session['net_err'] += 1
        session['last_response'] = f"Error: {str(e)[:30]}"   # <-- added
        return
    ...
    session['last_response'] = last   # <-- store response
    ...
    if any(x in last_l for x in ["thank", "success", "Thank you", "Thank You", "complete", "succeeded", "donation"]):
        ...
        session['cvv_err'] += 1
        session['declined'] += 1
    # ========== FIXED HERE: Supports network_error (underscore) AND network error (space) ==========
...
def update_ui_thread(chat_id, msg_id, session):
    while session['is_running']:
        ...
        # ======== MODIFIED UI TEXT (added Response line) ========
        text = f"""<b>Arise</b>
