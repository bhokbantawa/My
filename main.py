...
from gatet0 import Tele  # original gatet0.py – unchanged
...
def send_card_result(chat_id, cc, status_label, response, amount, elapsed):
    """Send a formatted result for non‑declined cards only."""
...
def check_card(cc, session):
    ...
    # Count every card attempt immediately
    session['processed'] += 1
    ...
    if proxy_url:
        ...
        print("[PROXY] No proxy available, using direct connection")
    ...
    try:
        ...
        session['last_cc'] = cc
        send_card_result(session['chat_id'], cc, "🌐 NETWORK ERROR", f"Error: {str(e)[:30]}", "0.50", "0.0")
        set_proxy(None)
        return
    finally:
        set_proxy(None)  # clear proxy
    ...
    # Determine status_label and send message if not plain DECLINED
    send_message = False
    if any(x in last_l for x in ["thank", "success", "Thank you", "Thank You", "complete", "succeeded", "donation"]):
        ...
        # Generic decline – no message
        session['declined'] += 1
        status_label = "❌ DECLINED"
        send_message = False

    if send_message:
        send_card_result(session['chat_id'], cc, status_label, last, amt, rt)
...
def single_check(message):
    ...
    def do_check():
        ...
        set_proxy(proxy_url)
        if proxy_url:
            print(f"[PROXY] Using {proxy_url} for {card}")
        try:
            raw = Tele(card)
            elapsed = round(time.time() - start_time, 1)
            # Classify
